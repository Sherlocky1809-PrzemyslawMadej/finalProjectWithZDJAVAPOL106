...
@AutoConfigureTestDatabase // Przygotowanie szablonu bazy danych H2 na potrzeby testu
@AutoConfigureMockMvc // Konfiguracja MockMVC
...
//    TO SĄ TESTY KOMPLEKSOWE, PRZEKROJOWE
    ...
    void when_get_sorted_asc_organization_in_db_will_return_OK() throws Exception {
        // given
        organizationRepository.saveAll(Arrays.asList(new Organization("Amazon"), new Organization("BCrypt"),
                new Organization("Crypto")));
        // when&then
        mockMvc.perform(MockMvcRequestBuilders.get("/api/organizations?sortType=ASC")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.jsonPath("$", hasSize(3)))
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].name", equalTo("Amazon")))
                .andExpect(MockMvcResultMatchers.jsonPath("$[1].name", equalTo("BCrypt")))
                .andExpect(MockMvcResultMatchers.jsonPath("$[2].name", equalTo("Cryptoreturn")))
                .andExpect(MockMvcResultMatchers.status().is2xxSuccessful());
    }
}
